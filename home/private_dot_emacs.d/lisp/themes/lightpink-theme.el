;;; lightpink-theme.el --- Custom theme for faces  -*- lexical-binding:t -*-
...
(let ((class '((class color) (min-colors 89))))
  (custom-theme-set-faces
   'lightpink
   `(default ((,class (:background "#f7eaf4" :foreground "black"))))
   `(cursor ((,class (:background "red"))))
   `(fringe ((,class (:background "gray85"))))
   ;; Highlighting faces
   `(highlight ((,class (:background "#ffbbe8"))))
   `(region ((,class (:background "#ffd8f2"))))
   ...
   ;; Mode line faces
   `(mode-line ((,class (:background "#ff66cc" :foreground "black" :box (:line-width -1 :style released-button)))))
...
(provide-theme 'lightpink)

;;; lightpink-theme.el ends here
