...
def main():
        ...
        for track in tracks:
                ...
                if tag == None or len(tag) == 0:
                        ...
                        tag = tag[0].replace(' ', '').lower()

                for splittag in tag.split(','):
                        count = count + 1
                        if data.get(splittag) == None: # create category if empty
                        	data[splittag] = 0
                        data[splittag] = data[splittag] + 1 # increment category

        # more filtering
        #del data['none']
        other = 0
        for category in data:
                if data[category] * 20 < count: # tags <5% of lib are 'other'
